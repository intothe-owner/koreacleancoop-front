...
useEffect(() => {
  ...
  const fetchPosts = async (currentPage: number, searchQuery: string, isAppend: boolean = false) => {
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/boards/${boardId}/posts?page=${currentPage}&limit=${listCount}&search=${searchQuery}`);
      ...
      <div className="max-w-6xl mx-auto px-4 w-full">
        ...
        {boardType === 'GALLERY' && (
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {posts.map((post: any, index: number) => (
              <Link key={post.id} href={`/boards/${boardId}/${post.id}`} ref={index === posts.length - 1 ? lastPostElementRef : null} className="group flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-all duration-300">
